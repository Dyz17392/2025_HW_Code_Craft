...
//近期存储的每种tag被访问的次数
vector<int> req_tag;
//每个时间片发起的读请求，元素[i][j]表示时间片i+1发起的第j+1个读请求编号
vector<unordered_set<int>> req_time;

vector<int> more_tooken;//额外tooken
...
void timestamp_action()
{
    ...
    //让disk进入新时间片待命：（这一步是为了更新DISK参数）
    update_timesample(disk, more_tooken, timestamp);
    fflush(stdout);
}
...
void delete_action()
{
    ...
    for (auto it = pendingRequests.begin();it != pendingRequests.end();) {
        if (delete_Set.count(request[*it].object_id)) {
            ...
            clear_request_id(disk, object,timestamp,request, *it);
            req_time[request[*it].start_time - 1].erase(*it);
...
void Ndisk_read(vector<int>& finishedThisTurn,int time) {
    DISK::choose temp = get_nearest_disk(disk, time, request);
    DISK* chosed_disk = temp.disk;//当前选择操作的硬盘（地址）
    int head_id = temp.head_id;//当前选择的磁头索引（0或1）
    while (chosed_disk != nullptr) {
        int target = (chosed_disk->head(head_id) + chosed_disk->get_d(head_id)) % V;
        std::pair<int, int> tem = chosed_disk->get_save_pos(target);
        int obj_idx = tem.first, block_idx = tem.second;
        bool read_successful;
        if (chosed_disk->head(head_id) == target) {
            read_successful = chosed_disk->read_act(timestamp, request, head_id);
        }
        else {
            if (chosed_disk->pass_head(target, time, request, head_id)) {
                read_successful = chosed_disk->read_act(timestamp, request, head_id);
            }
            else {
                read_successful = false;
                //if (target != chosed_disk->head(1 - head_id))
                chosed_disk->jump(target, time, request, head_id);
            }
        }
        if (read_successful) {
            //如果读成功了，需要更新参数
            for (int req_id : chosed_disk->get_request_id(target)) {
                ...
                request[req_id].check_done();
                if (request[req_id].is_done) {
                    finishedThisTurn.push_back(req_id);
                    //pendingRequests中清除值为req_id的元素
                    pendingRequests.erase(req_id);
                    req_time[request[req_id].start_time - 1].erase(req_id);
                    req_tag[object[request[req_id].object_id].tag - 1]--;
                }
            }
            //将该请求已读的对象块从DISK中弹出
            pop_Request_out(disk, chosed_disk, object, timestamp, request, target);
        }
        else {
            //没成功，说明这个选择的硬盘已经没法儿读了，可以进入下一个循环了
            chosed_disk->flag(head_id) = false;
        }
        temp = get_nearest_disk(disk, time, request);
        chosed_disk = temp.disk;
        head_id = temp.head_id;
    }
}
...
void read_action(int timestamp)
{
    ...
    for (int i = 1; i <= n_read; i++) {
        ...
        req_time[timestamp - 1].insert(request_id);
        //将请求压入
    ...
    for (int d = 0;d < N;d++) {
        disk[d].check_conflict(timestamp, request);
    }
    ...
    for (size_t i = 0;i < finishedThisTurn.size();i++) {
        ...
        total_scores += func(timestamp - request[finishedThisTurn[i]].start_time) * (request[finishedThisTurn[i]].object_size + 1) / 2;
    }
    fflush(stdout);
}
...
int main()
{
    ...
    int l = (T + EXTRA_TIME) % FRE_PER_SLICING == 0 ? (T + EXTRA_TIME) / FRE_PER_SLICING : (T + EXTRA_TIME) / FRE_PER_SLICING + 1;
    more_tooken = vector<int>(l);
    for (int i = 0; i < l; ++i) {
        int temp = 0;
        scanf("%d", &temp);
        more_tooken[i] = temp;
    ...
    req_time = vector<unordered_set<int>>(T + EXTRA_TIME);
    // 输出"OK"表示预处理阶段完成
    printf("OK\n");
    fflush(stdout);

    int next_check_time = EXTRA_TIME;/*下一个检测时间片*/
    // 主循环：从时间片1到T + EXTRA_TIME
    for (int t = 1; t <= T + EXTRA_TIME; t++) {
        timestamp_action(); // 1. 时间片对齐
        //if (t == 21868) {
        //    int ans = disk[0].read_farest_distance(disk[0].head(1), 7);
        //}
        delete_action();    // 2. 删除操作
        write_action(write_freq, write_num_every1800, delete_num_every1800);// 3. 写入操作
        read_action(t);      // 4. 读取操作
        vector<int> busy_req;//当前时间片要上报的繁忙读请求序列
        if (t > EXTRA_TIME) {
            for (auto it = req_time[t - EXTRA_TIME - 1].begin();it != req_time[t - EXTRA_TIME - 1].end();it++) {
                busy_req.push_back(*it);
                req_tag[object[request[*it].object_id].tag - 1]--;
                pendingRequests.erase(*it);
                clear_request_id(disk, object, t, request, *it);
            }
            req_time[t - EXTRA_TIME - 1].clear();
        }
        std::cout << busy_req.size() << std::endl;
        for (int i = 0;i < busy_req.size();++i) {
            std::cout << busy_req[i] << std::endl;
        }
        if (t % FRE_PER_SLICING == 0) {
            ...
            for (int i = 0; i < N; i++) {
                //printf("0\n");
                vector<std::pair<int, int>> swaped_pair = disk[i].swap_K(K, object);
                std::cout << swaped_pair.size() << std::endl;
                for (int i = 0;i < swaped_pair.size();++i) {
                    std::cout << swaped_pair[i].first + 1 << " " << swaped_pair[i].second + 1 << std::endl;
                }
            }
            fflush(stdout);
        }
