...
public:
    ...
    friend void update_timesample(vector<DISK>& disks,const vector<int>& more_tooken,int time) {
        for (int i = 0;i < disks.size();++i) {
            // more_tooken[(time-1)/1800]
            disks[i].tooken_[0] = disks[i].G + more_tooken[(time - 1) / 1800], disks[i].tooken_[1] = disks[i].G + more_tooken[(time - 1) / 1800];
